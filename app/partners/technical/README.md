...
| GET | `/v1/technical/prices/stats` | `prices:read` | Cross-source **statistical summary** per (asset, currency) (mean/median/2σ/3σ trimmed/min/max), stale sources excluded, + Gerami's quote (`?asset=`, `?currency=`, `?max_age_seconds=`, `?role=` — all optional; omit `asset` for every asset) |
...
stale sources and Gerami excluded from the sample. It groups by (asset,
currency) and returns one item per pair, so it is a list like the quote feeds.
...
stay slugs on both sides — `?asset=gold-18k&currency=irt` — and `currency`
carries its `slug` (`irt`), its `code` (`IRT`) and its display `symbol`
(`تومان`).
