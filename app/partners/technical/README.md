...
|--------|------|-------|---------|
| GET | `/v1/technical/prices/platforms/latest` | `prices:read` | Latest quote per platform/asset/currency, **with bid/ask** (`?source=`, `?asset=`, `?currency=`, `?type=`, `?assets=`); pass `?asset=` for one asset across all platforms |
...
`platforms/latest` mirrors the copilot `GET /api/v1/prices/latest`
(platform-compare) — nested source/asset/currency refs + `is_single_rate` +
bid/ask. `stats` summarizes each source's clean `bid` and `ask` **separately**
