...
`XCU9999g`); `currency` carries its `slug` (`irt`), its `code` (`IRT`) and its
display `symbol` (`تومان`), and both refs carry `title_fa` and `title_en`.
Inputs are lowercase on both sides now — `?asset=gold-18k&currency=irt`.
...
- [GET /v1/technical/prices/stats](#get-v1technicalpricesstats) — cross-source statistical summary (`bid`/`ask`, separately) per asset/currency; `asset` optional
...
| `currency` | string | – | filter by currency **slug** — the lowercase code (e.g. `irt`, `irr`, `usd`) |
...
Cross-source **statistical summary** per (asset, currency), computed from the
...
| Query | Type | Default | Description |
|-------|------|---------|-------------|
| `asset` | string | – | asset slug — e.g. `gold-18k` (طلا), `silver-999` (نقره), `copper` (مس). **Omit for every asset** |
| `currency` | string | – | currency **slug** — the lowercase code, e.g. `irt`. **Omit for every currency** |
...
Both filters are optional, exactly like the quote feeds. The response is a
standard list: **one item per (asset, currency) pair**. A statistic is only
meaningful inside one pair — a Toman gram price cannot be averaged with a Dollar
ounce price — so `gold-18k`/`irt` and `gold-ounce`/`usd` are two separate items.
Pass `?asset=gold-18k&currency=irt` and you get a one-item list.
...
# every asset in every currency it is quoted in
curl -H "X-API-Key: $KEY" "https://api.gerami.online/v1/technical/prices/stats"
```
...
```json
{
  ...
  "generated_at": "2026-07-19T15:32:26.000000+00:00"
}
```
...
- **Gerami is excluded from the statistics** and returned under `gerami` (its own
  ...
  market's stats for that side). It is `null` for a pair Gerami does not quote —
  e.g. `gold-ounce`/`USD`.
- **`params`** sits on the `data` block, not on each item: `max_age_seconds` and
  `role` apply to every pair in the response.
- A pair with no matching row simply isn't in `items`; an unknown `asset` gives
  `"items": [], "count": 0`.
...
> **Changed** — `asset` and `currency` used to be required and the response was a
> single object. They are now optional and the response is the standard list
> (`items` + `count`). A caller that passed both reads `data.items[0]` where it
